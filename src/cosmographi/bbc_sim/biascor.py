...
def build_biascor(
    key, alpha, beta, sigma_int, sigma_obs, sigma_x1_obs, sigma_c_obs,
    threshold, scale, w_fid, O_fid, 
    n=900_000, z_lo=0.02, z_hi=1.5, M0_ref=-19.3, M0=-19.3, ref=True):
    """
    ...
    sim = pop.simulate_salt2_population(
        ksim, z=z, mu_true=mu_true, alpha=alpha, beta=beta,
        sigma_int=sigma_int, sigma_obs=sigma_obs, M0_ref=M0_ref, M0=M0,
        sigma_x1_obs=sigma_x1_obs, sigma_c_obs=sigma_c_obs, ref=ref
    )
...
def build_bias_tools_cached(sigma_int, sigma_obs, sigma_x1_obs, sigma_c_obs,
                             threshold, scale, w_fid, O_fid, valid_counts, M0_ref, M0,
                             n=900_000, ref=True):
    ...
    for a_val in alpha_nodes:
        ...
        for b_val in beta_nodes:
            ...
            bc_node = build_biascor(subkey, alpha=a_val, beta=b_val,
                                    sigma_int=sigma_int, sigma_obs=sigma_obs, n=n,
                                    sigma_x1_obs=sigma_x1_obs, sigma_c_obs=sigma_c_obs,
                                    threshold=threshold, scale=scale,
                                    w_fid=w_fid, O_fid=O_fid, M0_ref=M0_ref, M0=M0, ref=ref)
