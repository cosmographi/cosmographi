...
def logpost(mu_true, mu_star, full_mask, C_diag_inv, params, w_lo, w_hi, O_fid, O_sigma, om_lo, om_hi):
    return log_prior(params, w_lo, w_hi, O_fid, O_sigma, om_lo, om_hi) + log_likelihood(mu_true, mu_star, full_mask, C_diag_inv)





def logpost_unconstrained(mu_star, full_mask, C_diag_inv, z, u,
                           w_lo, w_hi, O_fid, O_sigma, om_lo, om_hi):
    params = {
        "om": to_constrained(u["u_om"], om_lo, om_hi),
        "w":  to_constrained(u["u_w"],  w_lo,  w_hi),
    }
    log_jac = (log_abs_jacobian(u["u_om"], om_lo, om_hi)
               + log_abs_jacobian(u["u_w"], w_lo, w_hi))
