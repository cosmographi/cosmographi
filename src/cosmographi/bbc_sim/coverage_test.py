...
def run_sbc(
    ...
    def run_one_sim(z, mu_star, full_mask, init_pos, warmup_key, sample_key, ck):
        ...
        def run_one_chain(key, init_state, step_size, inverse_mass_matrix):
            kernel = blackjax.nuts(
                logpost_fn, step_size=step_size, inverse_mass_matrix=inverse_mass_matrix
            )
        ...
        positions, divergences = jax.vmap(run_one_chain)(
            sample_keys, init_states, step_sizes, inv_mass
        )
        ...
        idx = jax.random.choice(
            ck, om_chain.shape[0], shape=(n_posterior_samples,), replace=True, p=probs
        )
    ...
    failed_sims = [
        {"sim": i, "divergence_frac": float(div_fracs[i])}
        for i in range(n_sims)
        if not bool(ok_flags[i])
    ]
    if failed_sims:
    ...
    if n_dropped > 0:
        print(
            f"Dropping {n_dropped}/{s.shape[1]} sims with NaN samples "
